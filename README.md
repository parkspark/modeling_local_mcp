...
현재 설치된 `Pixal3D 1024` 자체는 이미지 입력만 지원하지만, GUI 프롬프트는 생성 후 Blender 후처리 계획으로 변환됩니다. 색상, 유광/무광, 금속성, 투명도, 전체 크기, 베벨, 스무딩, 폴리곤 감소를 지원하며 결과 GLB/BLEND/FBX에 반영됩니다. 프롬프트를 비워두면 기존 이미지 기반 생성과 변환만 수행합니다. `렌즈`, `안경테` 같은 부위 지정은 생성 결과의 오브젝트 또는 재질 이름에 `lens`, `frame`처럼 의미 있는 이름이 있을 때만 정확히 적용됩니다. 해석 결과는 작업 폴더의 `postprocess_plan.json`에 저장됩니다.
